...
API key 申请：[硅基流动](https://cloud.siliconflow.cn/)
...
| Agent | 职责 |
|---|---|
| Planner | 自动识别 10 个领域，用领域特定术语生成 2-4 个英文 query；结果不足 8 篇时反馈重搜 |
...
### 内置领域
`cv / nlp / multimodal / fluid / rl / robotics_uav / world_model / security / crypto / general_sci`

每个领域配对应的顶刊/顶会名（JFM、PoF、Nature、Nature Communications、ICML、CVPR、NeurIPS、IEEE S&P、USENIX Security、CCS、NDSS、CRYPTO、EUROCRYPT 等），用于检索词生成 + Writer 综述时识别顶级会议/期刊。
